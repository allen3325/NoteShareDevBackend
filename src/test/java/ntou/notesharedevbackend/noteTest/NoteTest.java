...
//    }
    ...
    public void testGetHotNote() throws Exception {
        ...
        mockMvc.perform(get("/note/hotNotes/" + offset + "/" + pageSize)
                        .headers(httpHeaders))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.res.items.[0].id").value(note.getId()))
                .andExpect(jsonPath("$.res.items.[0].type").value(note.getType()))
                .andExpect(jsonPath("$.res.items.[0].department").value(note.getDepartment()))
                .andExpect(jsonPath("$.res.items.[0].subject").value(note.getSubject()))
                .andExpect(jsonPath("$.res.items.[0].title").value(note.getTitle()))
                .andExpect(jsonPath("$.res.items.[0].professor").value(note.getProfessor()))
                .andExpect(jsonPath("$.res.items.[0].school").value(note.getSchool()))
                .andExpect(jsonPath("$.res.items.[0].likeCount").value(note.getLikeCount()))
                .andExpect(jsonPath("$.res.items.[0].favoriteCount").value(note.getFavoriteCount()))
                .andExpect(jsonPath("$.res.items.[0].unlockCount").value(note.getUnlockCount()))
                .andExpect(jsonPath("$.res.items.[0].downloadable").value(note.getDownloadable()))
                .andExpect(jsonPath("$.res.items.[0].commentCount").value(note.getCommentCount()))
                .andExpect(jsonPath("$.res.items.[0].price").value(note.getPrice()))
                .andExpect(jsonPath("$.res.items.[0].tag.[0]").value(note.getTag().get(0)))
                .andExpect(jsonPath("$.res.items.[0].tag.[1]").value(note.getTag().get(1)))
                .andExpect(jsonPath("$.res.items.[0].tag.[2]").value(note.getTag().get(2)))
                .andExpect(jsonPath("$.res.items.[0].publishDate").value(note.getPublishDate()))
                .andExpect(jsonPath("$.res.items.[0].description").value(note.getDescription()))
                .andExpect(jsonPath("$.res.items.[0].headerEmailUserObj.userObjEmail").value(appUser.getEmail()))
                .andExpect(jsonPath("$.res.items.[0].headerEmailUserObj.userObjName").value(appUser.getName()))
                .andExpect(jsonPath("$.res.items.[0].headerEmailUserObj.userObjAvatar").value(appUser.getHeadshotPhoto()))
